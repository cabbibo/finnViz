...
//p *= .4;
    ...
    float which = floor(uv.x+1 )/4;
   // uv.x = ( (uv.x+4000+1) % 4 ) - 2;//abs(uv.x) -5;

        float2x2 rr = Rot( time + p.z * 1 );
...
//
    ...
    for (float i = 0.; i < 4; i++) { 
        ...
        uv -= m * q;
        th += 0.5 * p.y *   (sin(time*.1+fn+20) + 1.1) + ( ( .5 + .3) * time * sin((which+1) * 100));
        uv = mul(Rot(th) ,uv);
        uv.x = abs(uv.x * 1)-.1*calmness;//-.1 * 1*(sin(uv.y*4));
        m *=.05*cos(8. * length(uv)) + (.25 * calmness + .5 );//.55;// +  0.2 * (sin(time * .1 + fn) + 1.1);// + 0.05 * cos(0.4 * p.y - 0.6 * _Time.y);
    ...
    for (float i = 0.; i < n; i++) { 
     ...
     d = length(uv) - 1* m;//smin(d,length(uv) - 3 * m,(sin(cid+time) * 4 + 6));
...
float2 calcIntersection( float3 ro ,  float3 rd ){     
    ...
    float ip = .000001;//* (1.1+sin(_Time.y));
    float h =  ip  * 4;
    float t = 0.0;
    float res = -1.0;
    float id = -1.0;
...
float3 render( float3 ro , float3 rd ){
    ...
    if( res.y > -.5 ){
        ...
        col *= 1-dot( -rd, nor);
                ...
                float2 uv2 = (fPos.xy * float2(1 , 2)) * .1+ .5;
    uv2 = clamp(uv2,0,1);
    ...
    col = lerp( col , col2 , saturate(res.y-1));
    if( res.y < 1 ){
      //  col = 0;
    }
